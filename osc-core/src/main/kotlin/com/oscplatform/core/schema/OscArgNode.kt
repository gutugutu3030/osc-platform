                ...
                is ArrayArgNode -> {
                    when (val length = node.length) {
                        ...
                        is LengthSpec.FromField -> {
                            ...
                            val referenced = args.take(index).firstOrNull { it.name == length.fieldName }
                                ?: throw IllegalArgumentException(
                                    "lengthFrom '${length.fieldName}' for '${node.name}' must reference a previous scalar LENGTH field in '$path'",
                                )

                            require(referenced is ScalarArgNode && referenced.role == ScalarRole.LENGTH && referenced.type == OscType.INT) {
                    ...
                    when (val item = node.item) {
                        ...
                        is ArrayItemSpec.TupleItem -> {
                            ...
                            val duplicateTupleNames = item.fields.groupBy { it.name }.filterValues { it.size > 1 }.keys
