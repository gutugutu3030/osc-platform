...
/**
 * tools/call の E2E 統合テスト。
 * stdioフレーム（Content-Length ヘッダ + JSON ペイロード）を入力として、
 * OscMcpServer の run() ループを通過し result / error レスポンスが返ることを検証する。
 */
class McpToolsCallIntegrationTest {
    ...
    /**
     * MCP stdio フレームの列をパースして ObjectNode のリストを返す。
     * Content-Length ヘッダを読み、指定バイト数のペイロードを取得する。
     */
    ...
    /**
     * McpAdapter の internal overload を呼びサーバを同期実行する。
     * 入力 JSON フレームを全て書き込み EOF にすることでサーバループを終了させる。
     */
