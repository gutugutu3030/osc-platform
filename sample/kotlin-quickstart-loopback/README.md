...
**v0.10.0-SNAPSHOT 以降:** `osc-gradle-plugin` による Kotlin クラス自動生成に対応しました。
...
`schema.kts` は `@OscSchemaDslMarker`（`@DslMarker`）によるスコープ制御に対応しています。
この sample では `build.gradle.kts` に `osc-script-definition` 依存を追加しているため、IDE で Gradle 同期を行うと `oscSchema {}` や `INT` などの DSL 記号が補完候補に表示されます。
詳しくは [osc-core/README.md](../../osc-core/README.md#ide-補完を有効にするためのセットアップ) を参照してください。

> 補完が反映されない場合は **Reload Gradle Project** を実行してから `schema.kts` を開き直してください。
