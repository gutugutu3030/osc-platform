 ...
 * の構築とバリデーションを検証するテスト。
 *
 * バンドルは複数のメッセージをアトミックに送信するためのグルーピング定義である。 同一バンドル内で引数名が重複すると初期化時にエラーとなる。
 */
class OscSchemaDslBundleTest {
  ...
  fun bundleCanReferenceByMessageName() {
    val schema = oscSchema {
      ...
      bundle("my_bundle") { message("set_light_color") }
    }
  ...
  fun schemaNoBundlesHasEmptyList() {
    val schema = oscSchema { message("/msg") { scalar("v", INT) } }
