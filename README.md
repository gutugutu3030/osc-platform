...
| `osc-adapter-mcp` | スキーマ → MCP tools 自動生成・MCP stdio / Streamable HTTP サーバ | [osc-adapter-mcp/README.md](osc-adapter-mcp/README.md) |
| `osc-adapter-webui` | `run` / `send` / `mcp` に埋め込まれる Web UI サーバ | [osc-adapter-webui/README.md](osc-adapter-webui/README.md) |
...
| `osc-script-definition` | `schema.kts` を IDE で認識させる Kotlin custom script definition | [osc-script-definition/README.md](osc-script-definition/README.md) |
| `sample/` | 各機能のサンプルプロジェクト集 | [sample/README.md](sample/README.md) |

## 技術スタック

<!-- version-sync:tech-stack:start -->
- Kotlin `2.1.20`
- JVM Toolchain `21`
...
<!-- version-sync:tech-stack:end -->

## スキーマ定義

> 📖 スキーマの書き方を詳しく学びたい場合は **[スキーマの書き方ガイド](docs/schema-guide.md)** を参照してください。  
> 予約語一覧・バリデーションルール・よくあるパターン集など、初心者向けに丁寧に解説しています。
...
- `schema.kts` の IDE 認識を安定させたい場合は `osc-script-definition` を依存へ追加し、Gradle 同期を実行してください
...
./osc-cli/build/install/osc-cli/bin/osc-cli run schema.yaml --webui --webui-port 19080
```
...
- `--webui`
- `--webui-port <httpPort>` (default: `8080`)
...
./osc-cli/build/install/osc-cli/bin/osc-cli send light.color --host 127.0.0.1 --port 9000 --webui
```
...
- `send` は送信先必須
  - `--host` 未指定: エラー
  - `--port` 未指定: エラー
- `--webui` を付けると即送信せず、ブラウザ UI を起動する
...
plugins {
    ...
    id("com.oscplatform.schema-codegen") version "0.10.0-SNAPSHOT"
}

dependencies {
    implementation("com.oscplatform:osc-core:0.10.0-SNAPSHOT")
    implementation("com.oscplatform:osc-transport-udp:0.10.0-SNAPSHOT")
}
...
```bash
./osc-cli/build/install/osc-cli/bin/osc-cli mcp schema.yaml --host 127.0.0.1 --port 9000 --streamable-http-port 8081
```
...
`--streamable-http-port` を指定した場合のエンドポイントは `/mcp`、待受ホスト既定値は `127.0.0.1` です。
